...
def eta(first_stop, second_stop, route_map):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    temporary_stop = first_stop # Code starts with the departure zone.
    total_time = 0

    while temporary_stop != second_stop:
        route = (temporary_stop, second_stop) # Provides the tuple.
        if route in route_map:
            total_time += route_map[route]["travel_time_mins"] # Gets the minutes travelled.
            temporary_stop = second_stop # Temporary stop is now the final second stop, and therefore will end. The code will know that the destination has been reached.
        else:
            next_stop = 0 # This code happens if there is no direct travel between the first and second stop.
            for route in route_map:
                if route[0] == first_stop: # The next stop will become the new "first stop" until the second stop has been reached.
                    next_stop = route[1] 
                    break
            if next_stop != 0:
                total_time += route_map[route]["travel_time_mins"] # Will add the minutes travelled if the next stop has a value.
